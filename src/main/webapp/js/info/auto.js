    ...
    start: function(event){
        ...
        if(code != 13 && code != 38 && code != 40){
            ...
            for (var i = 0; i < valueArr.length; i++) {
                ...
                let category = valueArr[i].category;
                if(reg.test(value)){
                    var div = document.createElement("div");
                    div.className = "auto_out";
                    div.seq = category+"='"+value+"'";
                    div.index = div_index;
                    div.innerHTML = value.replace(reg,"<strong>$1</strong>");
                ...
                } else if (reg.test(category) && value == "") {
                    var div = document.createElement("div");
                    div.className = "auto_out";
                    div.seq = category;
                    div.index = div_index;
                    div.innerHTML = category.replace(reg,"<strong>$1</strong>");
                    this.autoObj.appendChild(div);
                    setClass.removeClass(this.autoObj,"hidden");
                    div_index++;
                    if(div_index == 1) {
